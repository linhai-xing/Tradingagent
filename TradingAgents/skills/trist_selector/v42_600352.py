"""v4.2 bypass X2 for 600352 浙江龙盛 — user confirms yesterday's turnover was high"""
...
def patched(self, data, sector=None, discipline=None):
    ...
    if hasattr(result, "forbidden_hits") and result.forbidden_hits:
        ...
        result.forbidden_hits = non_x2 if non_x2 else []
        result.vetoed = bool(non_x2)
    return result
...
ind.update({"ticker":"600352","name":"浙江龙盛","price":float(kline["close"].values[-1])})
...
print(f"\n  600352 浙江龙盛 @ {price:.2f} (今日{chg:+.2%}) [v4.2 X2 bypass]")
print(f"  MA5:{ma5:.2f} MA10:{ma10:.2f} MA20:{ma20:.2f} 多头={ma5>ma10>ma20}")
print(f"  换手:{turnover:.2f}% | 5日均量:{avg_vol:.0f}万手 | 5日涨幅:{pre5d*100:+.1f}%")
...
if r.forbidden_hits: print(f"  Forbidden:{r.forbidden_hits}")
